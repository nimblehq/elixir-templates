defmodule Nimble.Elixir.Template.Addons.ObanTest do
  ...
  describe "#apply/2" do
    ...
    test "creates Oban migration file", %{
      ...
      in_test_project(test_project_path, fn ->
        ...
        assert_file("priv/repo/migrations/20201120074154_add_oban_jobs_table.exs", fn file ->
          assert file =~ """
                 defmodule NimbleElixirTemplate.Repo.Migrations.AddObanJobsTable do
                   use Ecto.Migration
    ...
    test "adds Oban configuration into the application.ex", %{
      ...
      in_test_project(test_project_path, fn ->
        ...
        assert_file("lib/nimble_elixir_template/application.ex", fn file ->
          assert file =~ """
                 {Oban, oban_config()}
                 """
          ...
          assert file =~ """
                   ...
                   defp oban_config do
                     Application.get_env(:nimble_elixir_template, Oban)
                   end
                 """
        end)
      end)
    ...
    test "adds Oban configuration into the config/config.exs", %{
      ...
      in_test_project(test_project_path, fn ->
        ...
        assert_file("config/config.exs", fn file ->
          assert file =~ """
                 config :nimble_elixir_template, Oban,
                   repo: NimbleElixirTemplate.Repo,
    ...
    test "adds Oban configuration into the config/test.exs", %{
      ...
      in_test_project(test_project_path, fn ->
        ...
        assert_file("config/test.exs", fn file ->
          assert file =~ """
                 config :nimble_elixir_template, Oban, crontab: false, queues: false, plugins: false
                 """
        end)
      end)
    end
    ...
    test "creates the worker folder", %{
      ...
      in_test_project(test_project_path, fn ->
        ...
        assert(File.dir?("lib/nimble_elixir_template_worker")) == true
      end)
    end
  end
end
