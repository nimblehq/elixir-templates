defmodule Nimble.Elixir.Template.Addons.DockerTest do
        ...
        assert_file("docker-compose.dev.yml", fn file ->
          assert file =~ """
                 ...
                 services:
                   db:
                     image: postgres:12.3
                     container_name: nimble_elixir_template_db
                     environment:
                       - POSTGRES_HOST_AUTH_METHOD=trust
                       - POSTGRES_DB=nimble_elixir_template_dev
                     ports:
                       - "5432:5432"
                 """
        end)
    ...
    test "copies the Dockerfile", %{
      ...
      in_test_project(test_project_path, fn ->
        ...
        assert_file("Dockerfile", fn file ->
          ...
          assert file =~
                   "COPY --from=build --chown=app_user:app_group /app/_build/prod/rel/nimble_elixir_template ./"
        end)
      end)
    end
    ...
    test "copies the bin/start.sh", %{
      ...
      in_test_project(test_project_path, fn ->
        ...
        assert_file("bin/start.sh", fn file ->
          assert file =~ """
                 bin/nimble_elixir_template eval "NimbleElixirTemplate.ReleaseTasks.migrate()"

                 bin/nimble_elixir_template start
                 """
        end)
      end)
    end
